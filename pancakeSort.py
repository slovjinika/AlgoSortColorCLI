...
class Sorting:
    ...
    def shuffle(self, arr):
        n = len(arr)
        for i in range(0, n):
            r = random.randint(0, n - 1)  # Use random.randint
    ...
    def strandSort(self, arr):
        n = len(arr)
        for i in range(n):
            # Find the start of the strand
            if i > 0 and arr[i] == arr[i - 1]:
                continue
            # Find the end of the strand
            j = i + 1
            while j < n and arr[i] == arr[j]:
                j += 1
            # Move the strand to the correct position
            k = i
            while k < j and k < n - (j - i):
                arr[k], arr[k + (j - i)] = arr[k + (j - i)], arr[k]
                k += 1
                self.print_square(arr)  # Assuming you have a print_square function
                time.sleep(0.1)
        return arr
...
#sorted_blocks = sorter.strandSort(color_blocks)
end_time = time.time()
